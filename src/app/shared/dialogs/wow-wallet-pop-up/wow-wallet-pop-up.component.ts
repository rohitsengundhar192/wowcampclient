...
export class WowWalletPopUpComponent implements OnInit {
  ...
  iframeLoaded() {
    ...
    iframe.addEventListener("load", (e) => {
      let value: any = {
        ...
        purchase_ticket_type: 0, //make dynamic ticket based on wow or volunteer
  ...
  //       total_debit_amount: parseInt(this.ticketprice),
  //       debit_entry_amount: parseInt(this.ticketprice),
  ...
  //   window.addEventListener('message', (e) => {
  //     if (e.origin === this.iframeSource && !dataProcessed) {
  //       if (e.data) {
  //         // Set the flag to true after processing the data
  //         dataProcessed = true;
  //         console.log('message from child WOW Wallet', e.data);

  //         this._datashare.shareWowAmountgetId(e.data);

  //         this._datashare.share_wow_wmount_get_id_data
  //           .pipe(
  //             take(1) // or take(1)
  //           )
  //           .subscribe((res) => {
  ...
  //         if (e.data != undefined) {
  //           this._datashare.CheckWowWalletDataPassed(1);
  //         }
  //       }
  //     }
  //   });
