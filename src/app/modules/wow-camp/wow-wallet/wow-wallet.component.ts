...
@Component({
  selector: 'app-wow-wallet',
  templateUrl: './wow-wallet.component.html',
  styleUrls: ['./wow-wallet.component.scss']
})
export class WowWalletComponent implements OnInit {
 ...
 constructor(
   ...
   public _dialogRef: MatDialogRef<WowWalletComponent>,
   @Inject(MAT_DIALOG_DATA) public data: any,
   private _customSpinnerService: CustomSpinnerService
 ) {}
 //* -------------------------  Lifecycle Hooks  --------------------------*//
 async ngOnInit(): Promise<void> {
   console.log(this.data,'wowwallet');
   this.getster_id = this.data?.getster_id;
   this.user_id = this.data?.user_id;
   this.iframeSource = 'https://p28.getwow.education';
 ...
 onclick() {
  console.log(this.amt_success,'amt_successamt_success')
