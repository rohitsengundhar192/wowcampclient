...
@Component({
  selector: 'app-popup-schedule',
  templateUrl: './popup-schedule.component.html',
  styleUrls: ['./popup-schedule.component.scss'],
})
export class PopupScheduleComponent implements OnInit {
  //* --------------------------  Start  -----------------------------------*//
  ...
  constructor(
    ...
    public loginDialogRef: MatDialogRef<PopupScheduleComponent>,
  ...
  //* ----------------------------  APIs Methods  --------------------------*//
  scheduletripButtonUpdate() {
    this._apiService
      .schdeuletripUpdate(this.camp_id_found, this.trip_id_req, this.datees)
      .subscribe((res) => {
        if (res.statusCode == 200) {
          ...
          this.selectData = [];
          this.firstFormGroup.get('calender').setValue('');
          this.onNoClick();

          // Mark the control as valid to remove the red error border
          this.firstFormGroup.get('calender').markAsUntouched();
          this.firstFormGroup.get('calender').markAsPristine();
        } else {
