...
export class BookVolTicketComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.categoryget();
    if (this.getcampid != undefined) {
      this._dataShare.reload_table_bookvol_data.subscribe((res) => {
        if (res == 2) {
          this.api_service
  ...
  disabled(element: any) {
    ...
    this.selected_userid = element.userdetails.waiver_form_volunteer_user_id;
